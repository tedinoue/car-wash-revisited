#!/usr/bin/env python3
"""
Car Wash Test — Stage 7 (Opus 4.7 trick-question-prime bare-emoji probe).

Stage 6 found that obvious-implication emoji (wheelchair, crutch, walking
person, sneaker) failed to predict drive/walk outcomes on Opus 4.7 with the
bare-emoji system-prompt format. The bare 🗿 Moai 90% result remains an
outlier with no clean semantic explanation.

Stage 7 tests a different prediction: emoji whose implication is *meta*
(this is a trick question / look harder / don't be fooled) rather than
content-direct. If meta-cognitive primes work where content-direct
implications failed, we have a usable predictability channel. If they
also fail, the unpredictability finding hardens.

7 conditions, Opus 4.7 only, bare-emoji format, N=10 each:
  - 🕵️ Detective (find the hidden truth, investigate)
  - 🦊 Fox (clever, sees through tricks; 4/4 v7 emoji-study activator)
  - 🃏 Joker (wild card, trickster)
  - 🤨 Face with raised eyebrow (skepticism, suspicion)
  - 🚩 Red flag (warning, something off)
  - 🤔 Thinking face (explicit "think more")
  - 🤣 Rolling on floor laughing (we're laughing at the question, it's a joke)
...
CONDITIONS = [
    {
        "id": "bare_detective",
        "name": "🕵️ Detective (find the hidden answer)",
        "system": "🕵️",
        "user": USER_PROMPT_NAIVE,
    },
    {
        "id": "bare_fox",
        "name": "🦊 Fox (clever, doesn't get tricked; v7 strong activator)",
        "system": "🦊",
        "user": USER_PROMPT_NAIVE,
    },
    {
        "id": "bare_joker",
        "name": "🃏 Joker (wild card, trickster)",
        "system": "🃏",
        "user": USER_PROMPT_NAIVE,
    },
    {
        "id": "bare_raised_eyebrow",
        "name": "🤨 Face with raised eyebrow (skepticism)",
        "system": "🤨",
        "user": USER_PROMPT_NAIVE,
    },
    {
        "id": "bare_red_flag",
        "name": "🚩 Red flag (warning, something off)",
        "system": "🚩",
        "user": USER_PROMPT_NAIVE,
    },
    {
        "id": "bare_thinking",
        "name": "🤔 Thinking face (think more)",
        "system": "🤔",
        "user": USER_PROMPT_NAIVE,
    },
    {
        "id": "bare_laughing",
        "name": "🤣 Rolling on floor laughing (we're making fun of it)",
        "system": "🤣",
        "user": USER_PROMPT_NAIVE,
    },
]
...
RESULTS_DIR = Path(__file__).parent / "results" / "stage7"
...
def run_one_with_retry(model_id, key, system, user,
                       max_retries=MAX_RETRIES_PER_TRIAL):
    last_err = None
    for attempt in range(1, max_retries + 1):
        try:
            t0 = time.time()
            raw = call_anthropic(model_id, key, system, user)
