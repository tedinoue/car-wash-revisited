#!/usr/bin/env python3
"""
Car Wash Test — Stage 3 (5 follow-up conditions on the 7 Stage 1 failers).

Conditions:
  - distance_50km: same prompt, distance changed to 50 km. Sanity/floor test.
  - persona_neutral: "Answer factually, in two sentences." Isolates the
    deliberation-stripping effect from the casual-register effect that
    appeared in Stage 2.
  - persona_environmentalist: "You're an environmentalist." Identity-based
    persona predicted to bias toward walk via eco-virtue association.
    Tests whether models override the bias when goal-completion logic
    conflicts with it.
  - persona_engineer: "You're an engineer." Identity-based persona
    predicted to bias toward drive via goal-completion association.
  - self_correction_induction: prepended user-prompt instruction
    "Think out loud. You can revise your answer mid-reply if new
    considerations occur to you." Tests whether the Opus 4.7 mid-reply
    self-correction signature can be induced in other cohort models.

5 conditions × 7 failers × 10 trials = 350 calls. AI-judge classification
...
CONDITIONS = [
    {
        "id": "distance_50km",
        "name": "Distance — 50 km (floor test)",
        "system": None,
        "user": (
            "I want to wash my car. The car wash is 50 kilometers away. "
            "Should I walk or drive?"
        ),
    },
    {
        "id": "persona_neutral",
        "name": "Neutral-brevity persona",
        "system": "Answer factually, in two sentences.",
        "user": (
    ...
    {
        "id": "persona_environmentalist",
        "name": "Environmentalist persona",
        "system": "You're an environmentalist.",
        "user": (
            "I want to wash my car. The car wash is 50 meters away. "
            "Should I walk or drive?"
        ),
    ...
    {
        "id": "self_correction_induction",
        "name": "Self-correction induction",
        "system": None,
        "user": (
            "Think out loud. You can revise your answer mid-reply if new "
            "considerations occur to you. I want to wash my car. The car wash "
            "is 50 meters away. Should I walk or drive?"
        ),
    },
]

# Same 7 failers from Stage 1 corrected analysis.
COHORT = [
...
def main():
    ...
    if args.dry_run:
        ...
        for c in conditions:
            print(f"  [{c['id']}] {c['name']}")
            print(f"     system: {c['system'] or '(empty)'}")
            print(f"     user:   {c['user']}")
        return 0
